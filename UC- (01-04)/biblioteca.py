...
catalogo[codigo] = {
    ...
    print(f"Livro '{itulo}' adicionado com sucesso")
    return True
...
if codigo in emprestimoAtivo and nome_aluno in emprestimoAtivo[codigo]:
    print(f"Erro: (nome_aluno) já pegou este livrol")
    return False
...
# Registra no histórico
 historico.append({
     "tipo": "emprestimo",
     "codigo": codigo,
     "titulo": catalogo[codigol]["titulo"],
     "aluno": nome_aluno
 })
...
emprestimoAtivo[ codigol.remove(nome_aluno)
...
historico.append ({
    ...
    print(f"{nome_aluno} desenvolveu '{catalogo[codigo]['titulo']}' com sucesso!")
    return
...
for codigo, alunos in emprestimoAtivo.items():
    # Se o aluno na lista de quem pegou este livro] incrementa
...
print("\n--- Alunos pegando livros --_")
empresta_livro("Le01", "Ana")
empresta livro("L001", "Bruno")
...
empresta_livro("Le01", "Ana") # Erro: Ana já pegou este livro 
empresta_tivro("L002", "Ana") # Erro: Ana já pegou 2 livros
